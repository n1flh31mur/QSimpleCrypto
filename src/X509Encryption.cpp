    ...
    const long& notBefore, const long& notAfter)
{
    ...
    if (!X509_set_pubkey(x509, pkey.get())) {
    ...
    QMapIterator<QByteArray, QByteArray> i(additionalData);
    while (i.hasNext()) {
        /* Read next item in list */
        i.next();

        /* Set additional data */
        if (!X509_NAME_add_entry_by_txt(name, i.key().data(), MBSTRING_ASC, (unsigned char*)i.value().data(), -1, -1, 0)) {
    ...
    if (!keyFileName.isEmpty()) {
        ...
        if (!PEM_write_bio_PrivateKey(keyFile.get(), pkey.get(), cipher, reinterpret_cast<unsigned char*>(passphrase.data()), passphrase.size(), nullptr, nullptr)) {
    ...
    if (!certificateFileName.isEmpty()) {
        /* Write certificate file on disk */
