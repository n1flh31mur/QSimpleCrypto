...
BlockCipherEncryption::BlockCipherEncryption()
{
}

///
/// \brief BlockCipherEncryption::generate_random_bytes - generate random bytes. Can be used for 'Random delta (RD)'
/// \param size - size of random bytes
/// \return
///
QByteArray BlockCipherEncryption::generate_random_bytes(const int& size)
{
...
///
/// \brief BlockCipherEncryption::encrypt_aes_block_cipher
...
/// \return
///
QByteArray BlockCipherEncryption::encrypt_aes_block_cipher(const EVP_CIPHER* cipher, const EVP_MD* md,
...
///
/// \brief BlockCipherEncryption::decrypt_aes_block_cipher
/// \param chiper - can be used with openssl evp chipers (ecb, cbc, cfb, ofb, ctr) - 128, 192, 256. Example: EVP_aes_256_ecb()
/// \param md - hash algroitm (sha, md and etc). Example: EVP_sha512_256
/// \param key
...
/// \return
///
QByteArray BlockCipherEncryption::decrypt_aes_block_cipher(const EVP_CIPHER* cipher, const EVP_MD* md,
    unsigned char key[], unsigned char iv[],
    const int& rounds, const QByteArray& passphrase,
    const QByteArray& salt, QByteArray data)
{
