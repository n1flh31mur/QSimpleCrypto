...
public:
    ...
    ///
    /// \brief generateRandomSalt - Function generates random bytes by size.
    /// \param size - Size of generated bytes.
    /// \return Returns random bytes.
    ///
    [[nodiscard]] QByteArray generateSalt(const qint32& size);
    ...
    /// \return Returns decrypted data or "", if error happened.
    ///
    [[nodiscard]] QByteArray decryptAesBlockCipher(QByteArray data, QByteArray key, QByteArray iv = "",
        QByteArray password = "", QByteArray salt = "", const int& rounds = aes256Rounds,
        const EVP_CIPHER* cipher = EVP_aes_256_cbc(), const EVP_MD* md = EVP_sha512());
};
} // namespace QSimpleCrypto
